...
@Component({
  selector: 'app-vh-cambio-datos-boleta',
  templateUrl: './vh-cambio-datos-boleta.component.html',
  styleUrls: ['./vh-cambio-datos-boleta.component.css']
})
export class VhCambioDatosBoletaComponent implements OnInit {
  datePipe: DatePipe;
  /*
"Aprobaciones"
...
"Rechazado Contabilidad"
  */
  ...
  nombrearchivoPorAplicar: string;
  archivoPorAplicar: File;
  ...
  onArchivoDocPorAplicar(event: any) {
    this.archivoPorAplicar = event.target.files[0];
    const file: File = event.target.files[0];
    this.nombrearchivoPorAplicar = file.name;
  }
  ...
  constructor(private _location: Location,
    ...
    if (this.router.getCurrentNavigation()?.extras.state) {
      ...
      this.nombrearchivoPorAplicar = this.itemsolicitud.docPorPlicar;
  ...
  ProcederRegistro() {
    ...
    this.spinner.show();
    this._SGeneralService.RegistrarSolicitudBDGeneral(this.unidadSeleccionada, this.user, this.EstadoSolicitud, this.filasData, "3", this.AsesorSelected?.id.toString())
      .subscribe({
        next: data => {
          if (data.status == 200) {
  ...
  FinalizarPorAdv() {
    ...
    Swal.fire({
      ...
      html: `<br>
              Ingrese comentario: <br>
              <textarea id="comentariofinalizacion" class="swal2-textarea"></textarea>
            `,
      icon: 'info',
      showCancelButton: true,
      ...
      if (result.isConfirmed) {
        ...
        if (!motivoDevolucion) {
          ...
          this._SGeneralService.EnvioTesoreriaGeneral(this.itemsolicitud, this.nroNotaCredito, this.archivoNotaCredito,
            this.archivoPorAplicar, motivoDevolucion + "S_FinalizandoADV")
            .subscribe({
              next: data => {
                if (data.status == 200) {
