...
import { Solicitud, Solicitud2 } from '../services/request.model';
...
export class SolicitudPendienteComponent implements OnInit {
  ...
  constructor(private router: Router, private toaster: Toaster, private spinner: NgxSpinnerService, private requestService: RequestService) { }
 ...
 /*   this.FechaInicio.setDate(this.FechaInicio.getDate() - 15);
  ...
  displayedColumns: string[] = ["ID", 'TipoSoli', 'Cliente', 'Estado', 'SolicitudCreada', 'Aprobador', 'RechazadoPor', 'Motivo', 'Unidad', 'Sede', 'Accion'];
  ...
  getSolicitud() {
    ...
    this.spinner.show();
    this.requestService.getSolicitudAdv(idUser, fecInicio, fecFin, this.AprobadoPor?.id, this.EstadoSolicitud, "Pendiente", this.ClienteSelect, this.Otro).subscribe((response) => {
      this.spinner.hide();
  ...
  SendAdvDatos(item: Solicitud2) {
    ...
    switch (tipoSolicitud) {
      case 1:
        ...
        localStorage.setItem('Formactivo', 'OP. CON NOTA DE CREDITO - Anulación por anticipo para aplicar a nuevo comprobante');
        this.router.navigate(["/CN-NotaCredito-Nuevo-Comprobante/item"], navigationExtras);
        break;
      case 2:
        localStorage.setItem('Formactivo', 'OP. CON NOTA DE CREDITO - Anulación por anticipo para devolución al cliente / error de pago');
        this.router.navigate(["/CN-NotaCredito-Devolucion-Cliente/item"], navigationExtras);
        break;
      case 3: //APROBADO POR CONTABILIDAD
        localStorage.setItem('Formactivo', 'OP. CON NOTA DE CREDITO - Cambio de datos (boleta por factura, nombre, dirección etc.)');
        this.router.navigate(["/CN-NotaCredito-Datos-Boleta/item"], navigationExtras);
        break;
      case 4:
        localStorage.setItem('Formactivo', 'OP. CON NOTA DE CREDITO - Carta cruzada (para aplicar a nuevo comprobante)');
        this.router.navigate(["/CN-NotaCredito-Carta-Cruzada/item"], navigationExtras);
        break;
      case 5:
        localStorage.setItem('Formactivo', 'OP. CON NOTA DE CREDITO - Desistimiento de compra (con entrega) (vehículos)');
        this.router.navigate(["/CN-NotaCredito-Compra-CN-Entrega/item"], navigationExtras);
        break;
      case 6:
        localStorage.setItem('Formactivo', 'OP. CON NOTA DE CREDITO - Desistimiento de compra (sin entrega) (vehículos) tipo de NC');
        this.router.navigate(["/CN-NotaCredito-Compra-SN-Cruzada/item"], navigationExtras);
        break;
      case 7:
        localStorage.setItem('Formactivo', 'OP. CON NOTA DE CREDITO - Devolución por ítem (para aplicar a nuevo comprobante)');
        this.router.navigate(["/CN-NotaCredito-Devolucion/item"], navigationExtras);
        break;
      case 8:
        localStorage.setItem('Formactivo', 'OP. CON NOTA DE CREDITO - Solicitud Por descuento');
        this.router.navigate(["/CN-NotaCredito-Descuento/item"], navigationExtras);
        break;
      case 9:
        localStorage.setItem('Formactivo', 'OP. SIN NOTA DE CREDITO - Pago en exceso (Devoluciones de caja)');
        this.router.navigate(["/SN-PagoExceso/item"], navigationExtras);
        break;
      case 10:
        localStorage.setItem('Formactivo', 'OP. SIN NOTA DE CREDITO - Pago EPDP');
        this.router.navigate(["/SN-PartePago-Epdp/item"], navigationExtras);
        break;
      case 11:
        localStorage.setItem('Formactivo', 'OP. SIN NOTA DE CREDITO - Pago EPDP Parcial');
        this.router.navigate(["/SN-Pago-Epdp-parcial/item"], navigationExtras);
        break;
        case 13:
          localStorage.setItem('Formactivo', 'OP. CON NOTA DE CREDITO - Anulación por factura o boleta para devolución al cliente / error de pago');
          this.router.navigate(["/CN-NotaCredito-Devolucion-Factura-Cliente/item"], navigationExtras);
          break;
      default:
        console.log('-');
        break;
