...
@Component({
  selector: 'app-vh-devolucion',
  templateUrl: './vh-devolucion.component.html',
  styleUrls: ['./vh-devolucion.component.css']
})
export class VhDevolucionComponent implements OnInit {
...
ProcederRegistro(){
  ...
  this.spinner.show();
  this._SGeneralService.RegistrarSolicitudBDGeneral(this.unidadSeleccionada,this.user, this.EstadoSolicitud, this.filasData,"7" ,this.AsesorSelected?.id.toString())
    .subscribe({
      next: data => {
        if (data.status == 200) {
