...
@Component({
  selector: 'app-pago-exceso',
  templateUrl: './pago-exceso.component.html',
  styleUrls: ['./pago-exceso.component.css'],
})
export class PagoExcesoComponent implements OnInit {
  /*
"Aprobaciones"
...
"Rechazado Contabilidad"
  */
  ...
  Envio(paso: any) {
    switch (paso) {
      ...
      case 'paso3':
        ...
        //   'Aprobaciones',
        //   'Revision Contabilidad',
        //   'Enviado a contabilidad por Jefatura',
        //   'none'
        // );
        break;
      case 'paso4':
          ...
          if (result.isConfirmed) {
            ...
            if (!motivoDevolucion) {
              ...
              this.EnviarBD(
                'Revision Cumplimiento',
                'Revision Tesoreria',
                'Enviado a tesoreia por Cumplimiento',
                motivoDevolucion
              );
  ...
  EnviarBD(paso, estado, subestado, motivo) {
    ...
    this._SGeneralService
      .EnviarBDGeneral(this.user, paso, estado, subestado, motivo)
      .subscribe({
        next: (data) => {
          if (data.status == 200) {
            /*let artcl: IApiResponse = JSON.parse(JSON.stringify(data));
  ...
  `,
      ...
      if (result.isConfirmed) {
        ...
        if (!motivoDevolucion) {
          ...
          this.spinner.show();
          this._SGeneralService
            .ObservarTesoreriaGeneral(this.itemsolicitud, motivoDevolucion)
            .subscribe({
              next: (data) => {
                if (data.status == 200) {
                  let artcl: IApiResponse = JSON.parse(JSON.stringify(data));
  ...
  AplicarTesoreria() {  
   /* if(!this.archivoTesoreria){
    ...
    this.spinner.show();
    this._SGeneralService
      .AplicarTesoreriaGeneral(
        this.itemsolicitud,
        this.fechaAplicacion.toString(),
        this.numSap,
        this.archivoTesoreria
      )
      .subscribe({
        next: (data) => {
          this.spinner.hide();
