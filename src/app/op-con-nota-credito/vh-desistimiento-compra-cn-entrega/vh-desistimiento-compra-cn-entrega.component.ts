...
@Component({
  selector: 'app-vh-desistimiento-compra-cn-entrega',
  templateUrl: './vh-desistimiento-compra-cn-entrega.component.html',
  styleUrls: ['./vh-desistimiento-compra-cn-entrega.component.css']
})
export class VhDesistimientoCompraCnEntregaComponent implements OnInit {
...
"Rechazado Contabilidad"
  */
    ...
    if (this.router.getCurrentNavigation()?.extras.state) {
      ...
      switch (estadoSolicitud) {
        ...
        case 'Revision Tesoreria':
          //console.log("estado actual es  Revision Tesoreria  =======>>>>>>>");
          this.activeStepIndex = 6;
          this.EditAtribFinales = true;
          this.EditFormula = true;
          this.EditAtribFinales2 = false;
          break;
  ...
  ProcederRegistro() {
    this.spinner.show();
    this._SGeneralService.RegistrarSolicitudBDGeneral(this.unidadSeleccionada, this.user, this.EstadoSolicitud, this.filasData, "5")
      .subscribe({
        next: data => {
          if (data.status == 200) {
  ...
  Envio(paso: any) {
    switch (paso) {
      ...
      `,
        ...
        //this.EnviarBD(paso,"Revision Contabilidad","Enviado a contabilidad por Gerencia",'none');
        break;
      case 'paso5':
  ...
  getListas(tabla: string): Observable<ListasModel[]> {

    var urlBase = `${environment.baseUrl}/api/listas`;
    return this.http
      .get(urlBase + '?table=' + tabla)
  ...
  listasForm() {
    this.spinner.show();
    this.getListas('Banco').subscribe((res: any) => {
      this.listsBanco = res;
    });
    this.getListas('FormaPago').subscribe((res: any) => {
