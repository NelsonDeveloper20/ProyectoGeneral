...
@Component({
  selector: 'app-vh-por-descuento',
  templateUrl: './vh-por-descuento.component.html',
  styleUrls: ['./vh-por-descuento.component.css']
})
export class VhPorDescuentoComponent implements OnInit {
...
ProcederRegistro(){
  ...
  this.spinner.show();
  this._SGeneralService.RegistrarSolicitudBDGeneral(this.unidadSeleccionada,this.user, this.EstadoSolicitud, this.filasData,"8",this.AsesorSelected?.id.toString())
    .subscribe({
      next: data => {
        if (data.status == 200) {
...
Envio(paso:any){
  switch (paso) {
    ...
    case 'paso2':
      ...
      break;
    case 'paso3':
    //this.EnviarBD(paso,"Revision Cumplimiento","Enviado a cumplimiento por Jefatura",'none');
  ...
  .then((result) => {
    if (result.isConfirmed) {
      ...
      if (!motivoDevolucion) {
        ...
        //this.EnviarBD(paso,"Revision Contabilidad","Aprobado por cumplimiento",motivoDevolucion);
        if(this._mostrarGerencia==true){
